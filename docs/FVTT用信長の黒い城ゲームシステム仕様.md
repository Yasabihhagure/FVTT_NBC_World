...
* **実装機能**:
    ...
    * **部隊士気チェック (Group Morale Check)**:
        ...
        * **前提条件**:
            * 対象のアクターが「フォルダ」に含まれていること（フォルダ単位でグループを認識するため）。
            * シーンが作成され、アクティブ（表示中/Viewed）であること。
            * 対象のメンバー（トークン）がそのシーン上に配置されていること。
        * **グループ判定ロジック**:
            * 実行したアクターと同じフォルダに含まれるアクターを「同グループ」とみなします。
            * フォルダに含まれていないアクターは部隊士気チェックの対象外となります。
